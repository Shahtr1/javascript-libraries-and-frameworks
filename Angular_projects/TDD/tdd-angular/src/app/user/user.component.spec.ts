...
describe('UserComponent', () => {
  ...
  it('sends request to get user data', () => {
    subscriber.next({ id: '1' }); // mimicking the page opened with specified url
  ...
  it('displays user name on page when user is found', () => {
    ...
    request.flush({ id: 1, username: 'user1', email: 'user1@mail.com' });
    fixture.detectChanges();

    const header = fixture.nativeElement.querySelector('h3');
    expect(header.textContent.trim()).toBe('user1');
  });

  it('displays error when user not found', () => {
    subscriber.next({ id: '2' }); // mimicking the page opened with specified url
    ...
    expect(alert.textContent.trim()).toBe('User not found');
  });
  ...
  it('displays spinner during user get request', () => {
    ...
    request.flush({ id: 1, message: 'user1', email: 'user1@mail.com' });
    fixture.detectChanges();
    expect(
      fixture.nativeElement.querySelector('span[role="status"]')
    ).toBeFalsy();
