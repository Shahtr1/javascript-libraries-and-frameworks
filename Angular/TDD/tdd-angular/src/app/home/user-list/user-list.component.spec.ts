...
const parsePageParams = (request: TestRequest) => {
  let size = Number(request.request.params.get('size'));
  let page = Number(request.request.params.get('page'));
...
describe('UserListComponent', () => {
  ...
  it('does not display next page button at last page', () => {
    const request = httpTestingController.expectOne(() => true);
  ...
  it('display previous page button in page 2', () => {
    const request = httpTestingController.expectOne(() => true);
